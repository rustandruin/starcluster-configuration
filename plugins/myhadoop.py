...
<configuration>
  <property>
    ...
    <value>hdfs://%(master_public)s:9000</value>
  </property>
  <property>
...
<?xml version="1.0"?>
 <configuration>
  ...
  <property>
    ...
    <value>%(master_public)s:8025</value>
  </property>
  <property>
    <name>yarn.resourcemanager.scheduler.address</name>
    <value>%(master_public)s:8030</value>
  </property>
  <property>
...
class HadoopInstaller(clustersetup.ClusterSetup):
    ...
    def _setup_hadoop_env(self, node, cfg):
        fname = posixpath.join(self.hadoop_home, 'etc/hadoop/hadoop-env.sh')
        lines = node.ssh.get_remote_file_lines(fname)
        fout = node.ssh.remote_file(fname, 'w')
        fout.writelines(lines)
        fout.write("export HADOOP_CLASSPATH=$HADOOP_CLASSPATH:$HADOOP_HOME/share/hadoop/tools/lib/*\n")
        fout.close()
...
""".format(user))
    ...
    def _configure_hadoop(self, master, nodes, user):
        ...
        cfg = {'master':master.alias, 
               'master_public':master.public_dns_name,
               'user':user,
               'replication':1,
               'hadoop_home' : self.hadoop_home,
               'hadoop_tmpdir': posixpath.join(self.hadoop_tmpdir, 'hadoop-%s' % user),
               'awskey': master.ec2.aws_access_key_id,
               'awssecretkey': master.ec2.aws_secret_access_key}
        ...
        for node in nodes:
            self.pool.simple_job(self._setup_hadoop_env, (node, cfg), jobid=node.alias)
        self.pool.wait(numtasks=len(nodes))
...
cp -r hadoop-dist/target/hadoop-2.7.0 {0}
chown -R :hadoop {0}
cd ~
rm -rf hadoop*
""" 
    ...
    def _chown_hadoop(self, node, user):
        node.ssh.execute("chown -R {0}:hadoop {1}".format(user, self.hadoop_home) )
    ...
    def run(self, nodes, master, user, shell, volumes):
        ...
        log.info("Job tracker status: http://%s:50030" % master.public_dns_name)
        log.info("Namenode status: http://%s:50070" % master.public_dns_name)
