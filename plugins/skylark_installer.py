...
# Autodetection of chipset seems to fail on EC2
    ...
    def _setup_lib_links(self, node):
        # the only way to get all the spark executors to know where the skylark libraries seems to be
        # to make a hard-link to the python interface and then add libcskylark to ld's cache
        installdir = self.installation_directories["SKYLARK_INSTALL_DIR"]
        instructions = [
            'ln -s {0}/lib/python2.7/site-packages/skylark /usr/lib/python2.7/dist-packages/skylark'.format(installdir),
            'echo "{0}/lib" > /etc/ld.so.conf.d/skylark.conf'.format(installdir),
            'ldconfig'
        ]
        self._follow_instructions(instructions, node)
